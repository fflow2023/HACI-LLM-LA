...
- 退出service目录，进入models/embedding目录，安装依赖(建议单独拉一个python环境安装，依赖挺容易出问题)
  ...
  ```
- 请自行下载或准备[paraphrase-multilingual-MiniLM-L12-v2](https://huggingface.co/sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2)模型文件，并将其完整放置到embedding目录下(即 `embedding/paraphrase-multilingual-MiniLM-L12-v2`)。
