...
### 任务四：知识库管理及API格式修改（zjh & zhy）
#### zjh
1. **修改知识库代码及向量持久化存储**  
   修改知识库的代码，并完成向量的持久化存储。

2. **确定向量存储文件的组织方式**  
   确定向量持久化存储时，存储的文件是如何组织的（放在哪个文件夹下的哪个位置，确定好即可）。

 zhy
1. **修改知识库检索API格式**  
   修改知识库检索时发送的API的格式，要求能选择指定知识库，然后把API的格式写在文档中（可以先写在README里面，也可以自己另起一个文件）。

2. **同步修改前端请求参数和格式**  
   同步修改前端发送请求时的参数和格式（可能需要等lsf把UI改好🤔）。

 任务五：语言学习助手及UI修改（lsf）
lsf
1. **增加语言学习助手的prompt**  
   增加日语学习助手的prompt和英语学习助手的prompt，多调试以优化效果。

2. **修改前端性格选项**  
   
   在前端界面中，把原来的四个性格的选项先注释掉（可以不用删），然后增加两个选项，分别是日语和英语。
